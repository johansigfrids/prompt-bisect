import { type OpenAIResponse, fetchOpenAICompletion } from './openai';
...
export const bisectPrompt = async (
  ...
  if (depth > MAX_RECURSION_DEPTH) {
    throw new Error(
      'Maximum recursion depth reached. Unable to bisect further.',
    );
  }
  ...
  const response: OpenAIResponse = await fetchOpenAICompletion(
    apiKey,
    model,
    prompt,
  );
  ...
  const firstResult = await bisectPrompt(
    apiKey,
    model,
    firstHalf,
    depth + 1,
  ...
  const secondResult = await bisectPrompt(
    apiKey,
    model,
    secondHalf,
    depth + 1,
  ...
  if (newlineCount >= 3) {
    ...
    const quarterResult = await bisectPrompt(
      apiKey,
      model,
      firstQuarter,
      depth + 1,
    ...
    const threeQuartersResult = await bisectPrompt(
      apiKey,
      model,
      remainingThreeQuarters,
      depth + 1,
      onProgress,
    );
